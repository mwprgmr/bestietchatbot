    ...
    try {
      ...
      validOrders.forEach((o: any) => {
        ...
        itemsList.forEach((i: any) => {
          orderKg += Number(i.quantity ?? i.quantity_kg ?? 0)
        })
      ...
      if (!itemsErr && itemsData) {
        ...
        itemsData.forEach((item) => {
          ...
          map[pName].kg += Number(item.quantity ?? item.quantity_kg ?? 0)
          map[pName].sales += Number(item.total ?? item.total_price ?? item.subtotal ?? 0)
        })
