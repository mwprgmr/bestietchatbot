-- ============================================================
-- BESTIET FRESH: MIGRATION 020 - FIX CREATE_ORDER_ATOMIC DB SCHEMA & COLUMN ALIGNMENT
-- ============================================================

-- Drop all existing overloads of create_order_atomic to prevent ambiguity
...
DROP FUNCTION IF EXISTS public.create_order_atomic(UUID, UUID, JSONB, DATE, TEXT);
...
DECLARE
    ...
    v_phone TEXT := p_customer_phone;
    v_final_address TEXT := p_delivery_address;
BEGIN
    ...
    IF v_final_address IS NULL AND p_address_id IS NOT NULL THEN
        SELECT address_line INTO v_final_address FROM public.addresses WHERE id = p_address_id;
    END IF;

    IF v_phone IS NULL AND p_customer_id IS NOT NULL THEN
        SELECT phone INTO v_phone FROM public.customers WHERE id = p_customer_id;
    END IF;

    -- 3. Idempotency Check
    IF p_idempotency_key IS NOT NULL AND p_idempotency_key <> '' THEN
        SELECT id, order_number, total_amount INTO v_order_id, v_order_number, v_grand_total
        FROM public.orders
    ...
    LOOP
        ...
        -- Support both weight_kg/quantity or direct quantity_kg
        v_weight_kg := COALESCE((v_item->>'weight_kg')::NUMERIC, (v_item->>'quantity_kg')::NUMERIC, 1.0);
    ...
    -- 7. Insert Order Record using actual columns
    ...
    LOOP
        ...
        -- Insert order item using actual columns
        ...
        SET available_stock = available_stock - v_total_kg,
            sold_stock = sold_stock + v_total_kg,
        ...
        INSERT INTO public.inventory_movements (
            ...
            COALESCE('Order ' || v_order_number || ' (' || p_source || ')', 'Idempotency: ' || COALESCE(p_idempotency_key, '')),
            v_order_id
        );
    END LOOP;
...
-- 2. Legacy 11-parameter overload for WhatsApp Bot & existing callers
CREATE OR REPLACE FUNCTION public.create_order_atomic(
    p_customer_id UUID,
    p_address_id UUID,
    p_items JSONB,
...
BEGIN
    RETURN public.create_order_atomic(
        ...
        p_customer_remarks := p_customer_remarks,
