-- Migration 027: Add product_name column to order_items, backfill names, and update create_order_atomic

-- 1. Add product_name column if not exists
ALTER TABLE public.order_items ADD COLUMN IF NOT EXISTS product_name TEXT;

-- 2. Backfill existing order_items with product names from products table
UPDATE public.order_items oi
SET product_name = p.name
FROM public.products p
WHERE oi.product_id = p.id AND (oi.product_name IS NULL OR oi.product_name = '');

-- 3. Update create_order_atomic to insert product_name into order_items
...
BEGIN
    ...
    LOOP
        ...
        INSERT INTO public.order_items (
            order_id, product_id, product_name, quantity, price_per_kg, cutting_type, total
        ) VALUES (
            v_order_id, v_product_id, v_product_name, v_total_kg, v_unit_price, v_cutting_type, v_item_subtotal
        );
