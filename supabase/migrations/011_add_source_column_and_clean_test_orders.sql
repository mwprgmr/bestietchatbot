-- Migration 011: Add source column to orders table, update create_order_atomic RPC, and flag test orders
-- Source values: 'whatsapp', 'admin', 'test'

-- 1. Add source column to orders if not present
ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS source TEXT NOT NULL DEFAULT 'whatsapp';

-- 2. Update existing test orders to source = 'test'
UPDATE public.orders
SET source = 'test'
WHERE customer_id IN (
  SELECT id FROM public.customers WHERE name LIKE '%Tester%' OR phone LIKE '91987654%' OR phone LIKE '918547395033'
) OR customer_remarks LIKE '%Tester%' OR order_number IN ('BF-20260831-7509', 'BF-20260831-2432', 'BF-20260831-2309', 'BF-20260831-1476');

-- 3. Update create_order_atomic PL/pgSQL RPC Function
...
DECLARE
    ...
    v_order_source TEXT := COALESCE(p_source, 'whatsapp');
...
BEGIN
    ...
    IF p_idempotency_key IS NOT NULL AND p_idempotency_key <> '' THEN
        ...
        WHERE reason LIKE '%' || p_idempotency_key || '%'
           OR reason = 'Idempotency: ' || p_idempotency_key
        LIMIT 1;
    ...
    LOOP
        ...
        -- Trust branch inventory price_per_kg first, fallback to item unit_price
        IF v_inv_price IS NOT NULL AND v_inv_price > 0 THEN
            v_unit_price := v_inv_price;
        ELSE
    ...
    -- Generate unique order number with current target inventory date (IST timezone)
    LOOP
    ...
    LOOP
        ...
        SELECT price_per_kg INTO v_inv_price
        FROM public.inventory
        WHERE product_id = v_product_id AND branch_id = v_target_branch AND inventory_date = v_target_date;
        ...
        INSERT INTO public.inventory_movements (
            ...
            COALESCE('Customer order ' || v_order_number || ' | Idempotency: ' || COALESCE(p_idempotency_key, ''), 'Idempotency: ' || COALESCE(p_idempotency_key, '')),
            v_order_id
        );
    END LOOP;
