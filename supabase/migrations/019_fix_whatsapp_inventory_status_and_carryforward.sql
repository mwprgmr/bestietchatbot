-- ============================================================
-- BESTIET FRESH: MIGRATION 019 - FIX WHATSAPP CHATBOT INVENTORY STATUS & AUTO CARRY-FORWARD
-- ============================================================

-- Drop existing overloads of create_order_atomic to prevent ambiguity
...
-- 1. Update get_available_branch_inventory to handle both 'available' and 'AVAILABLE' status values
...
BEGIN
    ...
    WHERE i.branch_id = p_branch_id
      ...
      AND (LOWER(i.status) = 'available' OR i.status = 'AVAILABLE')
...
-- 2. Update create_order_atomic RPC function with automatic inventory carryover fallback
...
BEGIN
    ...
    -- LOOP 1: Validate inventory and lock rows FOR UPDATE using (branch_id + product_id + inventory_date)
    ...
    LOOP
        ...
        -- AUTO CARRY-FORWARD FALLBACK: If target date has no inventory record yet, carry over latest available stock
        ...
        IF v_inv_id IS NULL THEN
            RAISE EXCEPTION 'NO_INVENTORY: Selected fish % is not available at % for %', v_product_name, v_branch_name, v_target_date;
        END IF;
    ...
    LOOP
        ...
        INSERT INTO public.inventory_movements (
            ...
            COALESCE('WhatsApp order ' || v_order_number, 'Idempotency: ' || COALESCE(p_idempotency_key, '')),
            v_order_id
        );
    END LOOP;
