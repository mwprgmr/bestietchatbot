...
function AdminLayoutInner({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const fetchStoreSettings = async () => {
      try {
        const { data } = await supabase.rpc('get_store_business_settings')
        if (data) {
          setStoreSettings({
            brandName: data.brand_name || 'BESTIET FRESH',
            tagline: data.tagline || 'Your Fresh Friend At The Door',
          })
        }
      } catch (e) {}
    }
  ...
  useEffect(() => {
    if (!loadingUser && currentUser?.isSuperAdmin) {
